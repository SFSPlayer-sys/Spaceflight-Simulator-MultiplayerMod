    ...
    public class WorldEventSyncing
    {
            ...
            public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
            {
                ...
                foreach (CodeInstruction code in instructions)
                {
                    ...
                    if (code.opcode == OpCodes.Stloc_0)
                    {
                        ...
                        // * Call `LocalManager.Update()` to process main thread actions.
                        yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(LocalManager), nameof(LocalManager.Update)));
                        // * Call `ChatWindow.Update()` to process main thread actions.
                        yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(ChatWindow), nameof(ChatWindow.Update)));
        ...
        public static class Rocket_OnFixedUpdate
        {
            public static bool Prefix(Rocket __instance)
            {
                ...
                int rocketId = LocalManager.GetSyncedRocketID(__instance);
                if (rocketId == -1)
                    return false;
