        ...
        public class SavingCache_SaveBuildPersistent
        {
            public static bool Prefix(SavingCache __instance, Blueprint new_BuildPersistent, bool cache)
            {
                if (ClientManager.multiplayerEnabled.Value)
                {
                    ...
                    {
                        Blueprint.Save(new DefaultFolder(Main.buildPersistentFolder.ToString(), null), new_BuildPersistent, Application.version);
                    });
                    return false;
                }
                return true;
                ...
                if (ClientManager.multiplayerEnabled.Value)
                {
                    ...
                    if (buildPersistent == null)
                    {
                            ...
                            thread = new Thread((ThreadStart)delegate
                        {
                            ...
                            IFolder folder = new DefaultFolder(path.ToString(), null);
                            if (path.FolderExists() && Blueprint.TryLoad(folder, logger, out Blueprint blueprint))
                            {
